 ...
 * @see Pepperjam_Affiliate_Helper_Map
 */
class Pepperjam_Affiliate_Helper_Map_Order
{
	/**
	...
	{
		return (int) Mage::helper('pepperjam_affiliate/config')->getTransactionType($params['store']);
	}
	/**
	 ...
	 * through to Pepperjam_Affiliate_Helper_Map::getDataValue, `$params`
	...
	public function getItemId($params)
	{
		return sprintf(
			$params['format'],
			preg_replace('/[^a-zA-Z0-9\-_]/', '', Mage::helper('pepperjam_affiliate/map')->getDataValue($params))
		);
	}
	...
	{
		$item = $params['item'];
		return Mage::helper('pepperjam_affiliate')->getCommissioningCategory($item);
	}
	...
	public function getNewToFile($params)
	{
		...
		return (int) Mage::helper('pepperjam_affiliate')->isNewToFile($order);
	}
}

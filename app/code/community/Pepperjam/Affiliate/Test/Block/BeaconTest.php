...
class Pepperjam_Affiliate_Test_Block_BeaconTest extends EcomDev_PHPUnit_Test_Case
{
	/**
	 ...
	 * @return Mock_Pepperjam_Affiliate_Helper_Config
	 */
	protected function _getConfigHelper($programId, $transactionType, $itemizeOrders)
	{
		$helper = $this->getHelperMock('pepperjam_affiliate/config', array(
	...
	public function testGetBeaconUrlBasic()
	{
		...
		$this->replaceByMock(
			'helper',
			'pepperjam_affiliate/config',
			$this->_getConfigHelper($programId, $transactionType, $itemizeOrders)
		);
		$helper = $this->getHelperMock('pepperjam_affiliate/data', array('buildBeaconUrl'));
		$helper->expects($this->once())
			->method('buildBeaconUrl')
			->with($this->identicalTo($params))
			->will($this->returnValue($beaconUrl));
		$this->replaceByMock('helper', 'pepperjam_affiliate', $helper);

		$block = $this->getBlockMock('pepperjam_affiliate/beacon', array('_getOrder'));
	...
	public function testGetBeaconUrlItemized()
	{
		...
		// the pepperjam_affiliate/data helper when called with the right params.
	...
	/**
	 * Test that Pepperjam_Affiliate_Block_Beacon::_getOrder will be invoked
	...
	public function testGetOrder()
	{
		...
		$beacon = $this->getBlockMock('pepperjam_affiliate/beacon', array());
	...
	/**
	 * Test that Pepperjam_Affiliate_Block_Beacon::showBeacon will return true
	...
	{
		$configHelper = $this->getHelperMock('pepperjam_affiliate/config', array('isEnabled', 'isConditionalPixelEnabled'));
		...
		$this->replaceByMock('helper', 'pepperjam_affiliate/config', $configHelper);

		$dataHelper = $this->getHelperMock('pepperjam_affiliate', array('isValidCookie'));
		...
		$beacon = $this->getBlockMock('pepperjam_affiliate/beacon', array('_getOrder'));

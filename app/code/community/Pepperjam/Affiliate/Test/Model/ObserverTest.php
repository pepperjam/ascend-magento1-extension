...
class Pepperjam_Affiliate_Test_Model_ObserverTest extends EcomDev_PHPUnit_Test_Case
{
	/**
	 * Test that Pepperjam_Affiliate_Model_Observer::createProductFeed
	 * will be invoked and expected the method
	 * Pepperjam_Affiliate_Model_Observer::getWebsitesDefaultStoreviews
	 * to be called and returned a known array of program keys map to default
	 * website store views, then expects the method
	 * Pepperjam_Affiliate_Model_Product::generateFeed to be invoked per
	...
	public function testCreateProductFeed()
	{
		...
		$helper = $this->getHelperMock('pepperjam_affiliate/data', array(
		...
		$productFeed = $this->getModelMockBuilder('pepperjam_affiliate/feed_product')
		...
		$this->replaceByMock('model', 'pepperjam_affiliate/feed_product', $productFeed);

		Mage::getModel('pepperjam_affiliate/observer')->createProductFeed();
	}
}

...
class Pepperjam_Affiliate_Block_Beacon extends Mage_Core_Block_Template
{
	/**
	...
	/** @var  Pepperjam_Affiliate_Helper_Data */
	protected $_helper = null;

	/** @var  Pepperjam_Affiliate_Helper_Config */
	protected $_configHelper = null;

	/**
	 * @return Pepperjam_Affiliate_Helper_Data
	 */
	protected function _getHelper()
	{
		if (!$this->_helper) {
			$this->_helper = Mage::helper('pepperjam_affiliate');
		}
	...
	/**
	 * @return Pepperjam_Affiliate_Helper_Config
	 */
	protected function _getConfigHelper()
	{
		if (!$this->_configHelper) {
			$this->_configHelper = Mage::helper('pepperjam_affiliate/config');
		}
	...
	public function getBeaconUrl()
	{
		...
		if ($order instanceof Mage_Sales_Model_Order) {
			if (Mage::helper('pepperjam_affiliate/config')->isItemizedOrders()) {
				$params = $this->_buildItemizedParams($order);
			} elseif (Mage::helper('pepperjam_affiliate/config')->isDynamicOrders()) {
				$params = $this->_buildDynamicParams($order);
			} else {
				$params = $this->_buildBasicParams($order);
			}

			$url = Mage::helper('pepperjam_affiliate')->buildBeaconUrl($params);
		}
		return $url;
	}
	...
	{
		$params = array(
			static::KEY_PID => Mage::helper('pepperjam_affiliate/config')->getProgramId(),
	...
	protected function _buildBasicParams(Mage_Sales_Model_Order $order)
	{
		...
		$params[static::KEY_TYPE] = Mage::helper('pepperjam_affiliate/config')->getTransactionType();
	...
	{
		$params = $this->_buildCommonParams($order);
		$params[static::KEY_INT] = Mage::helper('pepperjam_affiliate/config')->getInt();

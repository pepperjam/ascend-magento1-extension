...
class Pepperjam_Affiliate_Model_Feed_Product extends Pepperjam_Affiliate_Model_Feed_Abstract
{
	const DELIMITER = "\t";
	/**
	...
	{
		$store = $this->getStore();
		$callbackMap = Mage::helper('pepperjam_affiliate/config')->getCallbackMappings($store);
		return array_filter(array_map(
			function ($key) use ($callbackMap) {
				return isset($callbackMap[$key])? $callbackMap[$key]['column_name'] : null;
			},
			array_keys(Mage::helper('pepperjam_affiliate/config')->getProductFeedFields($store))
		));
	}
	/**
	...
	{
		$fields = array();
		$helper = Mage::helper('pepperjam_affiliate/config');
		$store = $this->getStore();
	...
	/**
	 * @see Pepperjam_Affiliate_Model_Feed_Abstract::_getDelimiter
	 * @return string
	 * @codeCoverageIgnore
	 */

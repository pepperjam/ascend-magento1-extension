...
abstract class Pepperjam_Affiliate_Model_Feed_Abstract
{
	/**
	...
	{
		$fields = array();
		$mappings = Mage::helper('pepperjam_affiliate/config')->getCallbackMappings();
	 ...
	 * Mage::helper('pepperjam_affiliate/map')->getDataValue(
	 *     array('key' => 'increment_id', 'item' => $item)
	 * );
	 * </code>
	 *
	 * @see src/app/code/community/EbayEnterprise/Affiliate/etc/config.xml pepperjam_newtork/pepperjam_affiliate/feeds contains mappings used for the affiliate feeds
	...
	protected function _invokeCallback($callbackConfig, $item)
	{
		...
		if (method_exists($obj, $method)) {
			...
			throw new Pepperjam_Affiliate_Exception_Configuration(
				sprintf(
	 ...
	 * @param  array $callbackConfig
	 * @return self
	 * @throws Pepperjam_Affiliate_Exception_Configuration If callback configuration is not valid
	 */
	protected function _validateCallbackConfig($callbackConfig)
	{
		if (empty($callbackConfig)) {
			throw new Pepperjam_Affiliate_Exception_Configuration('Callback configuration is empty or missing.');
		}
	...
	protected function _generateFilePath()
	{
		return self::normalPaths(
			Mage::getBaseDir(),
			Mage::helper('pepperjam_affiliate/config', $this->getStore())->getExportFilePath(),
			$this->_getFileName()
		);
	}
	/**

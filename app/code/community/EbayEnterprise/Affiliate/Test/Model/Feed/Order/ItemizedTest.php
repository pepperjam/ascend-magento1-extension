	...
	extends EcomDev_PHPUnit_Test_Case
{
	...
	{
		$format = '%s_itemized_orders_%s.csv';
		$programId = 'PROGRAM_ID';
		$time = 1397829577;
		$formattedTime = date('YmdHis', $time);
		$fileName = "{$programId}_itemized_orders_{$formattedTime}.csv";
		...
		$feed = Mage::getModel('eems_affiliate/feed_order_itemized', array('store' => $store, 'start_time' => $time));

		$config = $this->getHelperMock('eems_affiliate/config', array('getItemizedOrderFeedFileFormat', 'getProgramId'));
		...
		$config->expects($this->any())
			->method('getItemizedOrderFeedFileFormat')
